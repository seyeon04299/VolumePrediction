...
class EncoderDecoder(nn.Module):
    def __init__(self, d_model, h, attn_type, num_class, embedding_dim, d_ff, dropout, N, predict_steps, cov_dim):
        super(EncoderDecoder, self).__init__()
        c = copy.deepcopy
        self.position = PositionalEncoding(d_model)
        self.emb = Embedding(num_class,embedding_dim, cov_dim, d_model, self.position)
        self.attn = MultiHeadedAttention(d_model,h,attn_type)
        self.ff = PositionwiseFeedForward(d_model,d_ff,dropout)
        self.encoderlayer = EncoderLayer(d_model,c(self.attn),c(self.ff),dropout)
        self.decoderlayer = DecoderLayer(d_model,c(self.attn),c(self.attn),c(self.ff),dropout)
        self.encoder = Encoder(N, self.encoderlayer)
        self.decoder = Decoder(N,self.decoderlayer)
    ...
    def encode(self, x, idx):
        ...
        embeded = self.emb(x, idx)                  # torch.Size([32, 168, 24])
...
class Embedding(nn.Module):
    def __init__(self, num_class, embedding_dim, cov_dim, d_model, position):
    ...
    def forward(self, x, idx):
        ...
        ## Add differences of each x values to place attention to degree of change change
        vols = x[:,:,0].reshape(x.shape[0],x.shape[1],1)
        diffs_attn = vols.diff(axis = 1)
        diffs_attn = torch.cat((diffs_attn,diffs_attn[:,-1,:].reshape(diffs_attn.shape[0],1,diffs_attn.shape[2])),axis=1)
        try:
            output = torch.cat((x, diffs_attn ,onehot_embed), dim=-1)
...
def attention(query, key, value, attn_type, mask=None, dropout=None, alpha=None):
...
class MultiHeadedAttention(nn.Module): 
    def __init__(self, d_model, h, attn_type, dropout=0.2):  # TODO : h , dropout
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x, self.scores, self.attn = attention(query, key, value, self.attn_type, mask=mask, 
...
class Discriminator_WGAN(nn.Module):
    def __init__(self, train_window, rnn=False):
...
def test(model, predict_steps, device, predict_start, x, v_batch, id_batch):
    batch_size = x.shape[0]
    sample_mu = torch.zeros(batch_size, predict_steps, device=device)
    sample_q90 = torch.zeros(batch_size, predict_steps, device=device)
    src_mask, memory = model.encode(x[:, :predict_start,:], id_batch)
    for t in range(predict_steps):
        ys = x[:, predict_start:predict_start+t+1,:]
