...
class EmbeddingNetwork(torch.nn.Module):
    ...
    def forward(self, X, T):
        ...
        # print('X.shape : ', X.shape)                          #torch.Size([32, 300, 1])
        ...
        # print('X_packed[0].shape : ', X_packed[0].shape)        #torch.Size([9600, 1])
...
class TimeGAN(torch.nn.Module):
    ...
    """
    def __init__(self, feature_dim,Z_dim, hidden_dim, max_seq_len, num_layers, padding_value ):
    ...
    def _discriminator_forward(self, X, T, Z, gamma=1):
        ...
        return(H,E_hat,H_hat,Y_real,Y_fake,Y_fake_e)
    ...
    def _generator_forward(self, X, T, Z, gamma=1):
        ...
        return(H,H_hat_supervise,E_hat,H_hat,X_hat,Y_fake,Y_fake_e)
    ...
    def forward(self, X, T, Z, obj, gamma=1):
        ...
        if obj == "autoencoder":
            # Embedder & Recovery
            H, X_tilde,H_hat_supervise = self._recovery_forward(X, T)
            return H, X_tilde,H_hat_supervise

        elif obj == "supervisor":
            # Supervisor
            H, H_hat_supervise = self._supervisor_forward(X, T)
            return H, H_hat_supervise
            ...
            # Generator
            H,H_hat_supervise,E_hat,H_hat,X_hat,Y_fake,Y_fake_e = self._generator_forward(X, T, Z)
            return H,H_hat_supervise,E_hat,H_hat,X_hat,Y_fake,Y_fake_e
            ...
            # Discriminator
            H,E_hat,H_hat,Y_real,Y_fake,Y_fake_e = self._discriminator_forward(X, T, Z)
