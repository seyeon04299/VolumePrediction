...
class BaseTrainer_Autoformer:
    """
    Base class for Autoformer
    """
    def __init__(self, model_G, criterion, metric_ftns, optimizer_G, config):
        self.config = config
        ...
        if config.resume_G is not None:
            self._resume_checkpoint(config.resume_G)
