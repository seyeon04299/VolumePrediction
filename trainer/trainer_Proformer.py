...
class Trainer_Proformer(BaseTrainer_Proformer):
    """
    Trainer class
    """
    def __init__(self, model_G0, model_G1, model_G2, model_G3, model_G4, model_G5, model_G6,
        ...
        self.train_metrics = MetricTracker(
            'MSEloss_whole',
            'MSEloss0','MSEloss1','MSEloss2','MSEloss3','MSEloss4','MSEloss5','MSEloss6',
            'MSEloss_vol0','MSEloss_vol1','MSEloss_vol2','MSEloss_vol3','MSEloss_vol4','MSEloss_vol5','MSEloss_vol6',
            *[m.__name__ for m in self.metric_ftns], writer=self.writer
        )
        self.valid_metrics = MetricTracker('MSEloss_whole',
            'MSEloss0','MSEloss1','MSEloss2','MSEloss3','MSEloss4','MSEloss5','MSEloss6',
            'MSEloss_vol0','MSEloss_vol1','MSEloss_vol2','MSEloss_vol3','MSEloss_vol4','MSEloss_vol5','MSEloss_vol6', *[m.__name__ for m in self.metric_ftns], writer=self.writer)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (batch_x, batch_y, batch_x_mark, batch_y_mark,stat1,stat2,stat5,stat10,stat30,stat60) in enumerate(self.data_loader):
            ...
            MSElosses_step = [MSEloss0, MSEloss1, MSEloss2, MSEloss3, MSEloss4, MSEloss5, MSEloss6]
            ...
            for step in range(1,prediction_steps+1):
                ...
                if step==1:
                    ...
                    batch_x1 = torch.cat((batch_x1, outputs), 1)
                    ...
                    if prev_sec!=sec_step:
                        ...
                        batch_x1 = unnormalize_then_normalize(batch_x1,mean_prev,std_prev,mean_step,std_step, prev_sec, sec_step)
                        # print('batch_x1.shape : ', batch_x1.shape)
                ...
                outputs, vol_predicted, vol_real, _, _, MSEloss_vol1, MSEloss_lob1 = self.step_forward(step, batch_x1, batch_x_mark1, batch_y1, batch_y_mark1, step_seq_len, step_label_len, step_pred_len, model)
                ...
                self.MSElosses[step-1,batch_idx] = MSElosses_step[step-1].item()
                ...
                MSEloss_whole += lambda_step * MSElosses_step[step-1]
                prev_sec = sec_step

            self.MSEloss_whole.append(MSEloss_whole.item())
            
            ### Backwards with the whole mse loss
            if self.use_amp:
                self.scaler.scale(MSEloss_whole).backward()
                for optimizer in self.optimizers:
                    self.scaler.step(optimizer)
                    self.scaler.update()
            else:
                MSEloss_whole.backward()
                for optimizer in self.optimizers:
                    optimizer.zero_grad()
                    optimizer.step()
            
            if batch_idx % self.log_step == 0:
                self.logger.debug('Train Epoch: {}/{} {} MSEloss_whole: {:.4f}'.format(
                    epoch,
                    self.epochs,
        ...
        MSElosses = np.mean(self.MSElosses,axis=1)
        MSElosses_vol = np.mean(self.MSElosses_vol,axis=1)
        

        
        loss = np.average(self.MSEloss_whole)
        self.train_metrics.update('MSEloss_whole', loss)
        self.train_metrics.update('MSEloss0', MSElosses[0].item())
        self.train_metrics.update('MSEloss1', MSElosses[1].item())
        self.train_metrics.update('MSEloss2', MSElosses[2].item())
        self.train_metrics.update('MSEloss3', MSElosses[3].item())
        self.train_metrics.update('MSEloss4', MSElosses[4].item())
        self.train_metrics.update('MSEloss5', MSElosses[5].item())
        self.train_metrics.update('MSEloss6', MSElosses[6].item())

        self.train_metrics.update('MSEloss_vol0', MSElosses_vol[0].item())
        self.train_metrics.update('MSEloss_vol1', MSElosses_vol[1].item())
        self.train_metrics.update('MSEloss_vol2', MSElosses_vol[2].item())
        self.train_metrics.update('MSEloss_vol3', MSElosses_vol[3].item())
        self.train_metrics.update('MSEloss_vol4', MSElosses_vol[4].item())
        self.train_metrics.update('MSEloss_vol5', MSElosses_vol[5].item())
        self.train_metrics.update('MSEloss_vol6', MSElosses_vol[6].item())

        

        log = self.train_metrics.result()
        whole_volume = torch.cat((batch_x1, outputs), 1)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (batch_x, batch_y, batch_x_mark, batch_y_mark,stat1,stat2,stat5,stat10,stat30,stat60) in enumerate(self.valid_data_loader):
                ...
                MSElosses_whole = []
                MSElosses_vol = np.zeros((prediction_steps, self.len_epoch))
                MSElosses = np.zeros((prediction_steps, self.len_epoch))
                ...
                for step in range(1,prediction_steps+1):
                    ...
                    MSElosses_vol[step-1,batch_idx] = MSEloss_vol1.item()
                    MSElosses[step-1,batch_idx] = MSElosses_step[step-1].item()
                    ...
                    MSEloss_whole += lambda_step * MSElosses_step[step-1]
                    prev_sec = sec_step
                MSElosses_whole.append(MSEloss_whole.item())
            
            MSElosses = np.mean(MSElosses,axis=1)
            MSElosses_vol = np.mean(MSElosses_vol,axis=1)

            loss = np.average(MSElosses_whole)

                
        
        self.valid_metrics.update('MSEloss_whole', loss.item())
        self.valid_metrics.update('MSEloss_whole', loss)
        self.valid_metrics.update('MSEloss0', MSElosses[0].item())
        self.valid_metrics.update('MSEloss1', MSElosses[1].item())
        self.valid_metrics.update('MSEloss2', MSElosses[2].item())
        self.valid_metrics.update('MSEloss3', MSElosses[3].item())
        self.valid_metrics.update('MSEloss4', MSElosses[4].item())
        self.valid_metrics.update('MSEloss5', MSElosses[5].item())
        self.valid_metrics.update('MSEloss6', MSElosses[6].item())

        self.valid_metrics.update('MSEloss_vol0', MSElosses_vol[0].item())
        self.valid_metrics.update('MSEloss_vol1', MSElosses_vol[1].item())
        self.valid_metrics.update('MSEloss_vol2', MSElosses_vol[2].item())
        self.valid_metrics.update('MSEloss_vol3', MSElosses_vol[3].item())
        self.valid_metrics.update('MSEloss_vol4', MSElosses_vol[4].item())
        self.valid_metrics.update('MSEloss_vol5', MSElosses_vol[5].item())
        self.valid_metrics.update('MSEloss_vol6', MSElosses_vol[6].item())
    ...
    def step_forward(
        ...
        if self.use_amp:
            with torch.cuda.amp.autocast():
                if self.output_attention:
                    outputs = model(batch_x, batch_x_mark, dec_inp, batch_y_mark,step_label_len,step_pred_len)[0]
                else:
                ...
                MSEloss_vol = self.criterion(vol_predicted, vol_real)
        ...
        return outputs, vol_predicted, vol_real, lob_predicted, lob_real, MSEloss_vol, MSEloss_lob
