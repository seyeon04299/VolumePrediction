...
def main(config):
    ...
    if config['trainer']['criterion']=='GEV':
        criterion = None
    # criterion = None
    trainer = Trainer_Autoformer(model_G,
                        data_loader, 
                        valid_data_loader, 
                        criterion, 
                        metrics, 
...
if __name__ == '__main__':
    # ArgumentParser
    args = argparse.ArgumentParser(description='Autoformer')
    ...
    options = [
        ...
        CustomArgs(['--D_LAYERS', '--D_LAYERS'], type=int, target='arch_G;args;d_layers'),
